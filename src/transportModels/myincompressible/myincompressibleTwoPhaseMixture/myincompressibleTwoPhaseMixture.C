...
:
    ...
    lambda1_("lambda", dimensionSet( 1, 1, -3, -1, 0, 0, 0), nuModel1_->viscosityProperties().lookup("lambda")),
    lambda2_("lambda", dimensionSet( 1, 1, -3, -1, 0, 0, 0), nuModel2_->viscosityProperties().lookup("lambda")),
    beta1_("beta", dimensionSet( 0, 0, 0, -1, 0, 0, 0), nuModel1_->viscosityProperties().lookup("beta")),
    beta2_("beta", dimensionSet( 0, 0, 0, -1, 0, 0, 0), nuModel2_->viscosityProperties().lookup("beta")),
    U_(U),
    phi_(phi),
...
Foam::tmp<Foam::surfaceScalarField>
Foam::incompressibleTwoPhaseMixture::lambdaf() const
{
    const surfaceScalarField alpha1f
    (
        min(max(fvc::interpolate(alpha1_), scalar(0)), scalar(1))
    ...
    (
        ...
        (
            "lambdaf",
            alpha1f*lambda1_*fvc::interpolate(nuModel1_->nu())
          + (scalar(1) - alpha1f)*lambda2_*fvc::interpolate(nuModel2_->nu())
        )
    );
}
